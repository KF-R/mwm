...
def display_flag_sprites(canvas, x: int = 10, y: int = 10, padding: int = 10, scale: float = 1.0):
...
def display_high_scores(screen, high_scores, x, y, col_width, row_height, color=WHITE, bgcolor=BLACK, with_date: bool = True):
    ...
    rows.insert(1, '')  ## Blank line for spacing
...
def draw_death_clock(screen, x, y, radius: int = 32, value: int = 0, value_range: int = 60):
    ...
    # Calculate angle for timer dial
...
while True:
    ...
    if cycles // fall_cycles_per_tick == cycles / fall_cycles_per_tick:
        ## Player will fall if there's no ground beneath them
        if direction == 1:
        ...
        loot_list = [(loot_y + 1, loot_x, loot_type) if loot_y < GRID_HEIGHT - 1 and game_board[loot_y + 1][loot_x] == 0 else (loot_y, loot_x, loot_type) for loot_y, loot_x, loot_type in loot_list]
    ...
    for i, item in enumerate(loot_list):
        if item[0] == player_y and item[1] == player_x:
            loot_type = item[2]
            loot_list.pop(i)
            if loot_type == 4: 
                shields-= 1
